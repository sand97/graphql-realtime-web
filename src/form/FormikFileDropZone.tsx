...
export default function FormikFileDropZones(props: AssetDropZones) {
    ...
    const handleDeleteImage = (img: FileUploadType) => {
        if (imgIdToDelete.includes(img.id)) {
            ...
            setTimeout(() => setImgIdToDelete(imgIdToDelete.filter(id => id !== img.id)), 3000);
        }
    ...
    return (
        ...
        >
            <Box
                sx={[classes.dropZone, isDragActive && classes.dragActive,
                ...
                <Box component={'div'} textAlign={'center'} zIndex={4} position={'relative'}>
                    <React.Fragment>
                        ...
                        <Typography color={'primary'} fontSize={'14px'}
                                    sx={{mx: '10px'}}>
                            {t(props.onMultipleChange ? 'browser_multiple_file' : 'browser_one_file')}
            ...
            <Box sx={classes.imgContainer}>
                {
                    field.value && (Array.isArray(field.value) ? field.value : [
                        ...
                        <Button
                            ...
                            size={'small'}
                            variant={imgIdToDelete.includes(img.id) ? 'contained' : 'outlined'}
                            color={'error'}
                            sx={[classes.action, imgIdToDelete.includes(img.id) && classes.actionActive]}>
                            <DeleteIcon/>
                        </Button>
                    </Box>)
                }
